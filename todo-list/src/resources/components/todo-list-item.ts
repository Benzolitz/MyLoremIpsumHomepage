...
template.innerHTML = template.innerHTML = require('./todo-list-item.html');
...
export class TodoListItem extends HTMLElement {
	...
	renderChanges() {
		const text: any = this.shadowRoot.querySelector('li > span');
	...
	connectedCallback() {
		...
		const text: any = shadowDOM.querySelector('li > span');
		text.innerHTML = this.innerHTML;
		text.onclick = this.onDoneChange;

		// remove button action
		const button: any = shadowDOM.querySelector('li > button');
